<template>
  <div class="container mx-auto px-4 py-8">
    ...
    <p v-if="subcourseNameDisplay" class="text-lg mb-6 text-base-content text-opacity-80">
    ...
    <!-- Timeline Display -->
    <div v-else>
      <div v-if="Object.keys(groupedTimelines).length === 0" class="text-center italic py-6">
        No timeline entries found for this student in this group.
      </div>

      <!-- Loop through grouped timelines -->
      <div v-for="group in groupedTimelines" :key="group.scheduleInfo.id" class="mb-8 collapse collapse-arrow bg-base-200 rounded-box shadow">
        <input type="checkbox" class="peer" :checked="group.entries.length > 0" />
        <div class="collapse-title text-xl font-medium peer-checked:bg-base-300">
          {{ group.scheduleInfo.name || `Schedule ID ${group.scheduleInfo.id}` }}
           <span class="text-sm font-normal">({{ group.entries.length }} entries)</span>
        </div>
        <div class="collapse-content peer-checked:bg-base-100">
          <div v-if="group.entries.length > 0" class="overflow-x-auto mt-4">
            <table class="table table-sm w-full"> <!-- Added table classes -->
              <!-- Table Header -->
              <thead>
                <tr>
                  <th>Timestamp</th>
              ...
              <tbody>
                <tr v-for="entry in group.entries" :key="entry.id">
                  <!-- Timestamp Column -->
                  <td class="text-xs text-base-content text-opacity-70 whitespace-nowrap">
                  ...
                  <td>
                    <span class="badge badge-ghost badge-sm">{{ entry.subschedule }}</span>
                  </td>
                  <td>
                    ...
                    <a v-else-if="entry.notetype === 1"
                       ...
                       :title="isImageFile(entry.note) ? `Preview ${entry.note}` : `Download ${entry.note}`"
                    >
    ...
    <dialog id="image_preview_modal" class="modal" :open="isPreviewModalVisible">
      <div class="modal-box w-11/12 max-w-5xl">
        <h3 class="font-bold text-lg mb-4">Preview: {{ previewImageFilename }}</h3>

        <!-- Loading/Error inside Modal -->
        <div v-if="isPreviewLoading" class="text-center py-10">
          <span class="loading loading-spinner text-primary"></span> Loading image...
        </div>
        <div v-else-if="previewError" class="alert alert-warning text-sm p-2 my-2">
           <span>{{ previewError }}</span>
        </div>
        <div v-else-if="previewImageUrl">
          <img :src="previewImageUrl" alt="Image Preview" class="max-w-full max-h-[70vh] mx-auto object-contain" />
        </div>
...
const studentNameDisplay = ref(route.meta?.state?.studentName || route.params?.studentName || props.studentId);
...
const groupedTimelines = computed(() => {
  ...
  timelines.value.forEach(entry => {
    if (groups[entry.schedule_id]) {
      ...
      console.warn(`Timeline entry ${entry.id} references unknown schedule ID ${entry.schedule_id}`);
...
const fetchTimelineData = async () => {
  ...
  try {
    ...
    try {
      const subcourseResponse = await dataService.getSubcourse(props.subcourseId);
      subcourseInfo.value = subcourseResponse.data?.data || subcourseResponse.data;
      ...
      throw new Error(`Could not load course information for group ${props.subcourseId}.`);
    }
    ...
    schedules.value = schedulesResponse.data?.data || schedulesResponse.data || [];
    timelines.value = timelinesResponse.data?.data || timelinesResponse.data || [];

     // Basic validation/logging
     console.log(`Fetched ${schedules.value.length} schedules for course ${courseId}`);
     console.log(`Fetched ${timelines.value.length} timeline entries for student ${props.studentId} in group ${props.subcourseId}`);


  } catch (err) {
    console.error("Failed to load timeline data:", err);
    error.value.page = err.message || err.response?.data?.error || 'An unknown error occurred.';
...
.prose button.link { /* Ensure file links look good in prose */
