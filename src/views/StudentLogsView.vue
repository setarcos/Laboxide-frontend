  ...
  <div class="p-4 md:p-6">
      ...
      <div class="card-body">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
          <div>
            ...
            <input type="datetime-local" id="start_time" v-model="startTime" class="input input-bordered w-full" />
          </div>
          <div>
            <label class="label" for="end_time">
              <span class="label-text">End Time</span>
            </label>
            <input type="datetime-local" id="end_time" v-model="endTime" class="input input-bordered w-full" />
          </div>
          <button class="btn btn-primary md:mt-9" @click="fetchLogs" :disabled="isLoading || !startTime || !endTime">
            <span v-if="isLoading" class="loading loading-spinner loading-xs"></span>
            Fetch Logs
          </button>
        </div>
        <p v-if="filterError" class="text-error text-sm mt-2">{{ filterError }}</p>
      </div>
    </div>
    ...
    <div v-else-if="fetchError" class="alert alert-error shadow-lg">
      <div>
        ...
        <span>Error loading student logs: {{ fetchError.message || fetchError }}</span>
      </div>
    </div>

    <!-- Student Logs Table -->
    <div v-else-if="logs.length > 0" class="overflow-x-auto bg-base-100 rounded-box shadow">
      <table class="table table-zebra w-full">
        ...
        <tbody>
          <tr v-for="log in logs" :key="log.id">
            ...
            <td>
              <span :class="log.confirm ? 'badge badge-success' : 'badge badge-ghost'">
                {{ log.confirm ? 'Yes' : 'No' }}
              </span>
            </td>
            <td class="whitespace-pre-wrap max-w-xs truncate" :title="log.note">{{ log.note || '-' }}</td>
            <td>{{ log.tea_name || 'N/A' }}</td>
            <td class="whitespace-pre-wrap max-w-xs truncate" :title="log.tea_note">{{ log.tea_note || '-' }}</td>
          </tr>
        </tbody>
      </table>
    </div>
     <div v-else-if="searchedOnce && logs.length === 0" class="text-center italic py-10">
