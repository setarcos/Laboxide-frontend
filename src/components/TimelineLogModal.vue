    ...
    <div v-else>
      ...
      <div class="form-control mb-4">
        ...
        <select
          ...
          :disabled="allAvailableSchedules.length === 0"
        >
          ...
          <option
            v-for="sched in allAvailableSchedules"
            :key="sched.id"
            :value="sched.id"
          >
            Week {{ sched.week }}
      ...
      <div class="form-control mb-4">
        ...
        <div v-else-if="subSchedules.length > 0" class="space-y-1">
          <label
            v-for="step in subSchedules"
            :key="step.id"
            class="flex items-center cursor-pointer p-1.5 rounded hover:bg-base-200 border border-transparent has-[:checked]:border-primary has-[:checked]:bg-primary/10"
          >
            <input
              type="radio"
              name="timeline_step"
            ...
            <span class="flex-grow text-sm">
              <span
                class="font-mono bg-base-300 rounded px-1.5 py-0.5 mr-1.5 text-xs"
                >{{ step.step }}</span
              >
              {{ step.title }}
            </span>
...
const selectedScheduleObject = computed(() => {
  return (
    allAvailableSchedules.value.find(
...
const isLoggable = computed(() => {
  ...
  if (
    selectedStepValue.value === "other" &&
    customTitle.value.trim().length === 0
  ) {
    return false;
  }
  if (noteType.value === 0) {
...
const fetchSubSchedulesForSelectedSchedule = async () => {
  ...
  try {
    const subSchedulesResponse = await dataService.getSubSchedules(
      selectedScheduleId.value,
    );
    subSchedules.value = (subSchedulesResponse.data || []).sort(
      (a, b) => a.step - b.step,
    );
  } catch (err) {
    console.error(
...
const fetchData = async () => {
  ...
  try {
    ...
    const schedulesResponse = await dataService.getSchedules(
      props.subcourse.course_id,
    );
    allAvailableSchedules.value = (schedulesResponse.data || []).sort(
      (a, b) => a.week - b.week,
    );
    ...
    if (selectedScheduleId.value) {
      ...
      console.warn(
        `No schedules found for course ${props.subcourse.course_id}`,
      );
    }
...
const isStepLogged = (originalStepTitle) => {
  ...
  return existingTimelineEntries.value.some(
    (entry) =>
...
const findExistingEntryIdForLogTitle = (logTitle, scheduleIdToMatch) => {
  ...
  return entriesMatchingTitleAndSchedule.length > 0
    ? entriesMatchingTitleAndSchedule[0].id
    : null;
};
...
const submitTimelineEntry = async () => {
  ...
  if (
    ...
    selectedStepValue.value === "finish" ||
    !selectedScheduleId.value
  ) {
    console.warn(
  ...
  try {
    const logTitleToSave =
      selectedStepValue.value === "other"
        ? customTitle.value.trim()
        : selectedStepValue.value.trim();
    const targetScheduleId = selectedScheduleId.value; // Use the ID from the dropdown

    const entryToDeleteId = findExistingEntryIdForLogTitle(
      logTitleToSave,
      targetScheduleId,
    );
    ...
    if (entryToDeleteId) {
      ...
      try {
        ...
        existingTimelineEntries.value = existingTimelineEntries.value.filter(
          (entry) => entry.id !== entryToDeleteId,
        );
      } catch (deleteErr) {
        console.error(
          `Failed to delete previous entry ID ${entryToDeleteId}:`,
          deleteErr,
        );
...
watch(selectedScheduleId, async (newId, oldId) => {
  if (
    newId === oldId &&
    subSchedules.value.length > 0 &&
    !isFetchingSubSchedules.value
  )
...
.has-\[:checked\]\:border-accent:has(
    input[type="radio"][value="finish"]:checked
  ) {
  border-color: hsl(var(--a));
}
.has-\[:checked\]\:bg-accent\/10:has(
    input[type="radio"][value="finish"]:checked
  ) {
