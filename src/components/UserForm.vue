<template>
  <form @submit.prevent="submitForm" class="space-y-4 p-2">
    <div>
      ...
      <input id="user-id" type="text" v-model="formData.user_id" placeholder="Unique login identifier" class="input input-bordered w-full" :disabled="isEditing" required />
       <p v-if="isEditing" class="text-xs text-base-content/60 mt-1">User ID cannot be changed after creation.</p>
    </div>
    <div>
      <label class="label" for="user-username">
        <span class="label-text">Real Name</span>
      </label>
      <input id="user-username" type="text" v-model="formData.username" placeholder="User's full name" class="input input-bordered w-full" required />
    </div>
    <div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-2">
             <div class="form-control">
                <label class="label cursor-pointer justify-start gap-2">
                    <input type="checkbox" :checked="hasPermission(PERMISSION_ADMIN)" @change="togglePermission(PERMISSION_ADMIN)" class="checkbox checkbox-primary" />
                    <span class="label-text">Admin</span>
                </label>
            </div>
             <div class="form-control">
                <label class="label cursor-pointer justify-start gap-2">
                    <input type="checkbox" :checked="hasPermission(PERMISSION_TEACHER)" @change="togglePermission(PERMISSION_TEACHER)" class="checkbox checkbox-primary" />
                    <span class="label-text">Teacher</span>
                </label>
            </div>
             <div class="form-control">
                <label class="label cursor-pointer justify-start gap-2">
                    <input type="checkbox" :checked="hasPermission(PERMISSION_LAB_MANAGER)" @change="togglePermission(PERMISSION_LAB_MANAGER)" class="checkbox checkbox-primary" />
                    <span class="label-text">Lab Manager</span>
                </label>
            </div>
        </div>
         <p class="text-xs text-base-content/60 mt-1">Current permission value: {{ formData.permission }}</p>
    </div>
...
watch(() => props.initialData, (newData) => {
  if (newData) {
    // Ensure permission is a number
     formData.value = { ...newData, permission: Number(newData.permission) || 0 };
  } else {
