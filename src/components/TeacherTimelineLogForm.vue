...
<template>
  <form @submit.prevent="submitForm">
    <p class="text-sm mb-4">Adding log for student: <span class="font-semibold">{{ student.stu_name }} ({{ student.stu_id }})</span></p>

    <div class="form-control mb-3">
      <label class="label cursor-pointer justify-start gap-4">
        <span class="label-text text-sm">Type:</span>
        <span class="flex items-center gap-2 text-sm">
          <input type="radio" name="teacher_note_type" class="radio radio-sm" :value="0" v-model="noteType" /> Text Note
        </span>
        <span class="flex items-center gap-2 text-sm">
          <input type="radio" name="teacher_note_type" class="radio radio-sm" :value="1" v-model="noteType" /> File Upload
        </span>
      </label>
    </div>
    ...
    <div v-if="noteType === 0" class="form-control mb-4">
      <label class="label pb-1"><span class="label-text">Note Content:</span></label>
      <textarea
    ...
    <div v-if="noteType === 1" class="form-control mb-4">
      <label class="label pb-1"><span class="label-text">Select File:</span></label>
      <input
        type="file"
    ...
    <div class="modal-action mt-6">
      ...
      <button type="submit" class="btn btn-primary btn-sm" :class="{ 'loading': isSaving }" :disabled="isSaving || !isNoteValid">
...
const submitForm = async () => {
  if (!isNoteValid.value) {
    saveError.value = noteType.value === 0 ? "Note content cannot be empty." : "Please select a file to upload.";
    return;
  }
  isSaving.value = true;
